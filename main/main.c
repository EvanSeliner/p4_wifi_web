...
"    box.id='latency-ui';"
"    box.style.cssText='position:fixed;right:12px;bottom:12px;background:#111;color:#eee;padding:10px 12px;border-radius:8px;font:14px system-ui,Arial;box-shadow:0 2px 8px rgba(0,0,0,.3);z-index:2147483647';"
"    const row=document.createElement('div'); row.style.display='flex'; row.style.alignItems='center'; row.style.gap='8px';"
"    const led=document.createElement('span'); led.id='led'; led.style.cssText='display:inline-block;width:10px;height:10px;border-radius:50%;background:#666'; row.appendChild(led);"
"    const t1=document.createElement('span'); t1.textContent='RTT:'; row.appendChild(t1);"
"    const rtt=document.createElement('b'); rtt.id='rtt'; rtt.textContent='--'; row.appendChild(rtt);"
"    const t1u=document.createElement('span'); t1u.textContent=' ms'; row.appendChild(t1u);"
"    const t2=document.createElement('span'); t2.style.marginLeft='10px'; t2.textContent='count:'; row.appendChild(t2);"
"    const cnt=document.createElement('b'); cnt.id='count'; cnt.textContent='0'; row.appendChild(cnt);"
"    const t3=document.createElement('span'); t3.style.marginLeft='10px'; t3.textContent='WS:'; row.appendChild(t3);"
"    const wsEl=document.createElement('b'); wsEl.id='ws'; wsEl.textContent='OFF'; row.appendChild(wsEl);"
"    const t4=document.createElement('span'); t4.style.marginLeft='6px'; t4.textContent='msg:'; row.appendChild(t4);"
"    const wscEl=document.createElement('b'); wscEl.id='wsc'; wscEl.textContent='0'; row.appendChild(wscEl);"
"    const t5=document.createElement('span'); t5.style.marginLeft='10px'; t5.textContent='frames:'; row.appendChild(t5);"
"    const vfEl=document.createElement('b'); vfEl.id='vframes'; vfEl.textContent='0'; row.appendChild(vfEl);"
"    box.appendChild(row);"
"    const hint=document.createElement('div'); hint.id='lat-hint'; hint.style.opacity='.8'; hint.style.marginTop='4px'; hint.style.fontSize='12px'; hint.textContent='Press any gamepad button to measure round-trip'; box.appendChild(hint);"
"    const img=document.createElement('img'); img.id='cam'; img.alt='camera'; img.style.cssText='display:block;max-width:45vw;max-height:40vh;margin-top:8px;border-radius:8px'; box.appendChild(img);"
"    (document.body||document.documentElement).appendChild(box);"
"    let lastURL=null;"
"    const flash=(ok)=>{led.style.background=ok?'#0a0':'#a00'; setTimeout(()=>{led.style.background='#666';},120);};"
"    const ping=async(btn)=>{ const t0=performance.now(); try{ const res=await fetch('/pad',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({btn,pressed:true,t_client_ms:Math.round(t0)})}); const t1=performance.now(); const j=await res.json(); rtt.textContent=(t1-t0).toFixed(2); cnt.textContent=j.count||0; flash(true);}catch(e){ flash(false);} };"
"    let last=0; const poll=()=>{ const gps=navigator.getGamepads?navigator.getGamepads():[]; for(const g of gps){ if(!g) continue; for(let i=0;i<g.buttons.length;i++){ if(g.buttons[i].pressed){ const now=performance.now(); if(now-last>90){ last=now; ping(i);} break; } } } requestAnimationFrame(poll); }; requestAnimationFrame(poll);"
"    const proto=location.protocol==='https:'?'wss':'ws'; const ws=new WebSocket(proto+'://'+location.host+'/ws'); ws.binaryType='arraybuffer';"
"    ws.onopen=()=>{ wsEl.textContent='OPEN'; try{ws.send('ping');}catch(e){} };"
"    setInterval(()=>{ if(ws&&ws.readyState===1){ try{ws.send('ping');}catch(e){} } },2000);"
"    ws.onmessage=(ev)=>{ if(ev.data instanceof ArrayBuffer){ vfEl.textContent = (Number(vfEl.textContent)||0)+1; try{ const blob=new Blob([ev.data],{type:'image/jpeg'}); if(lastURL) URL.revokeObjectURL(lastURL); lastURL=URL.createObjectURL(blob); img.src=lastURL; }catch(e){} } else { wscEl.textContent = (Number(wscEl.textContent)||0)+1; } };"
"    ws.onerror=()=>{ wsEl.textContent='ERROR'; }; ws.onclose=()=>{ wsEl.textContent='CLOSED'; }; window._p4ws=ws;"
